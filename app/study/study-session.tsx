...
const QUALITY_BUTTONS: { quality: Quality; label: string; key: string; style: string }[] = [
  {
    quality: 0,
    label: 'Again',
    key: '1',
    style: 'bg-danger/10 text-danger border-danger/30 hover:bg-danger/20 hover:border-danger/50',
  },
  {
    quality: 2,
    label: 'Hard',
    key: '2',
    style: 'bg-orange-500/10 text-orange-300 border-orange-500/30 hover:bg-orange-500/20 hover:border-orange-500/50',
  },
  {
    quality: 4,
    label: 'Good',
    key: '3',
    style: 'bg-elevated text-t1 border-border-hi hover:bg-border-hi hover:border-accent/30',
  },
  {
    quality: 5,
    label: 'Easy',
    key: '4',
    style: 'bg-accent/10 text-accent border-accent/30 hover:bg-accent/20 hover:border-accent/60',
  },
]
  ...
  if (!cards.length) {
    return (
      <div className="text-center py-24 animate-fade-in">
        ...
        <p className="text-sm text-t2">No cards due today. Come back tomorrow.</p>
      </div>
    )
  }
      ...
      >
        ...
        >
          ...
          >
            ...
            <p className="text-4xl font-bold text-t1 text-center tracking-tight">{card.word}</p>
            <p className="text-xs text-t3 mt-8 flex items-center gap-1.5">
              <kbd className="px-1.5 py-0.5 rounded bg-elevated border border-border text-t3 font-mono text-xs">space</kbd>
              to reveal
            </p>
          </div>
          ...
          >
            <div>
              <p className="text-xs font-semibold text-t3 uppercase tracking-widest mb-2">Definition</p>
              <p className="text-base text-t1 leading-relaxed">{card.definition}</p>
            </div>
            {card.example && (
              <div>
                <p className="text-xs font-semibold text-t3 uppercase tracking-widest mb-2">Example</p>
                <p className="text-sm text-t2 italic leading-relaxed">"{card.example}"</p>
              </div>
            )}
            {card.notes && (
              <div>
                <p className="text-xs font-semibold text-t3 uppercase tracking-widest mb-2">Notes</p>
                <p className="text-sm text-t2 leading-relaxed">{card.notes}</p>
              </div>
            )}
        ...
        <div className="mt-5 animate-fade-up">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {QUALITY_BUTTONS.map(({ quality, label, key, style }) => (
              ...
              >
                <span>{label}</span>
                <span className="text-[10px] opacity-50 font-normal hidden sm:inline">[{key}]</span>
              </button>
            ))}
          </div>
        </div>
